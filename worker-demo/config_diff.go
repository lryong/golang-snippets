...
// oldList, a newList can ben obtained
func diffMirrorConfig(oldList, newList []mirrorConfig) []mirrorCfgTrans {
	...
	nList := make([]mirrorConfig, len(oldList))
	copy(oList, oldList)
	copy(nList, newList)
	...
	for i, j := 0, 0; i < len(oList) && j < len(nList); {
		...
		if n.Name < o.Name {
			...
			operations = append(operations, mirrorCfgTrans{diffDelete, 0})
			i++
		} else {
