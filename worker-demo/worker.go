...
// A Worker is a isntance of tunasync worker
type Worker struct {
	L    sync.Mutex
	cfg  *Config
...
// ReloadMirrorConfig refresh the providers and jobs from new mirror config
...
// makeHTTPServer Ctrl receives commands from the manager
func (w *Worker) makeHTTPServer() {
	...
	s.POST("/", func(c *gin.Context) {
		...
		// shedule should be flushed
		w.schedule.Remove(job.Name())

		//if jobs disabled, start them first
		switch cmd.Cmd {
...
func (w *Worker) runHTTPServer() {
	...
	if w.cfg.Server.SSLCert == "" && w.cfg.Server.SSLKey == "" {
		...
		if err := httServer.ListenAndServeTLS(w.cfg.Server.SSLCert, w.cfg.Server.SSLKey); err != nil {
			panic(err)
		}
	}
}
...
func (w *Worker) runSchedule() {
	...
	for {
		...
		case jobMsg := <-w.managerChan:
			...
			// syncing status is only meaningful when job is running. If it's paused or disabled a
			// sync failure signal would be emitted
			// which needs to be ignore
